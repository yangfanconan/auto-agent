...
╚═══════════════════════════════════════════════════════════╝
    ...
    def print_tools_status(self, tools: Dict):
        ...
        for name, info in tools.items():
            status = "[green]✅[/green]" if info.get("available", False) else "[red]❌[/red]"
    ...
    def get_user_input(self, prompt_text: str = "📝 任务指令") -> Optional[str]:
        ...
        try:
            user_input = Prompt.ask(
                f"\n[{self.theme_manager.colors.progress_primary}]{prompt_text}[/{self.theme_manager.colors.progress_primary}]"
            )
    ...
    def get_user_input_multiline(self, prompt_text: str = "输入内容") -> Optional[str]:
        """获取多行用户输入"""
    ...
    def select_option(
        ...
        for i, opt in enumerate(options):
            ...
            self.console.print(f"  [{self.theme_manager.colors.progress_primary}]{marker}[/{self.theme_manager.colors.progress_primary}] [{i}] {opt}")
