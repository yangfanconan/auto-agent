...
class TestTaskParser:
    ...
    def test_identify_task_types_environment(self):
        from core.task_parser import TaskParser, TaskType
        parser = TaskParser()
        types = parser._identify_task_types("安装依赖和环境配置")
        assert TaskType.ENVIRONMENT_SETUP in types

    def test_identify_task_types_documentation(self):
        from core.task_parser import TaskParser, TaskType
        parser = TaskParser()
        types = parser._identify_task_types("编写文档说明")
    ...
    def test_extract_title_description(self):
        from core.task_parser import TaskParser
        parser = TaskParser()
    ...
    def test_has_explicit_template(self):
        from core.task_parser import TaskParser
        parser = TaskParser()
        assert parser._has_explicit_template("简单实现一个功能") is True
        assert parser._has_explicit_template("完整实现一个功能") is False
...
class TestTaskPlan:
    ...
    def test_get_failed_count(self):
        ...
        subtask1 = SubTask(
            id="s1", name="s1", description="",
            task_type=TaskType.CODE_GENERATION, status="failed"
        )
        subtask2 = SubTask(
            id="s2", name="s2", description="",
            task_type=TaskType.TESTING, status="pending"
...
class TestTaskTracker:
    ...
    def test_init(self, temp_workspace):
        ...
        assert tracker.storage_dir == Path(temp_workspace)

    def test_register_plan(self, temp_workspace, sample_task_plan):
        from core.task_tracker import TaskTracker
        tracker = TaskTracker(storage_dir=temp_workspace)
    ...
    def test_start_task(self, temp_workspace, sample_task_plan):
        from core.task_tracker import TaskTracker
        tracker = TaskTracker(storage_dir=temp_workspace)
        tracker.register_plan(sample_task_plan)
    ...
    def test_update_subtask_progress(self, temp_workspace, sample_task_plan):
        from core.task_tracker import TaskTracker
        tracker = TaskTracker(storage_dir=temp_workspace)
        tracker.register_plan(sample_task_plan)
    ...
    def test_complete_subtask(self, temp_workspace, sample_task_plan):
        from core.task_tracker import TaskTracker
        tracker = TaskTracker(storage_dir=temp_workspace)
        tracker.register_plan(sample_task_plan)
    ...
    def test_fail_subtask(self, temp_workspace, sample_task_plan):
        from core.task_tracker import TaskTracker
        tracker = TaskTracker(storage_dir=temp_workspace)
        tracker.register_plan(sample_task_plan)
    ...
    def test_complete_plan(self, temp_workspace, sample_task_plan):
        from core.task_tracker import TaskTracker
        tracker = TaskTracker(storage_dir=temp_workspace)
        tracker.register_plan(sample_task_plan)
    ...
    def test_get_events(self, temp_workspace, sample_task_plan):
        from core.task_tracker import TaskTracker
        tracker = TaskTracker(storage_dir=temp_workspace)
        tracker.register_plan(sample_task_plan)
...
class TestTaskScheduler:
    ...
    def test_register_handler(self, temp_workspace):
        from core.scheduler import TaskScheduler
        from core.task_tracker import TaskTracker
...
class TestEnvironmentManager:
    ...
    def test_check_docker_support(self, temp_workspace):
        from modules.environment import EnvironmentManager
        manager = EnvironmentManager(workspace=temp_workspace)
        result = manager.check_docker_support()
        assert isinstance(result, bool)

    def test_scan(self, temp_workspace):
    ...
    def test_get_report(self, temp_workspace):
        from modules.environment import EnvironmentManager
        manager = EnvironmentManager(workspace=temp_workspace)
        manager.scan()
    ...
    def test_install_requirements_file_not_found(self, temp_workspace):
        from modules.environment import EnvironmentManager
        manager = EnvironmentManager(workspace=temp_workspace)
        result = manager.install_requirements("nonexistent.txt")
        assert result is False

    def test_run_in_docker(self, temp_workspace):
        from modules.environment import EnvironmentManager
        manager = EnvironmentManager(workspace=temp_workspace)
        success, output = manager.run_in_docker("python:3.10", "echo test")
        assert isinstance(success, bool)
...
class TestCodeGenerator:
    ...
    def test_init(self, temp_workspace):
        ...
        assert gen.workspace == Path(temp_workspace)
    ...
    def test_generate_basic_framework_python(self, temp_workspace):
        from modules.code_generator import CodeGenerator
        gen = CodeGenerator(workspace=temp_workspace)
    ...
    def test_generate_basic_framework_javascript(self, temp_workspace):
        from modules.code_generator import CodeGenerator
        gen = CodeGenerator(workspace=temp_workspace)
    ...
    def test_generate_basic_framework_other(self, temp_workspace):
        from modules.code_generator import CodeGenerator
        gen = CodeGenerator(workspace=temp_workspace)
...
更多文本"""
    ...
    def test_get_generated_files(self, temp_workspace):
        from modules.code_generator import CodeGenerator
        gen = CodeGenerator(workspace=temp_workspace)
    ...
    def test_basic_static_analysis(self, temp_workspace):
        from modules.code_generator import CodeGenerator
        gen = CodeGenerator(workspace=temp_workspace)
        code = "import *\ndef test(): pass"
...
class TestTestRunner:
    ...
    def test_init(self, temp_workspace):
        ...
        assert runner.workspace == Path(temp_workspace)
...
```"""
    ...
    def test_generate_basic_tests(self, temp_workspace):
        from modules.test_runner import TestRunner
        runner = TestRunner(workspace=temp_workspace)
...
class TestGitManager:
    ...
    def test_is_available(self, temp_workspace):
        from modules.git_manager import GitManager
        manager = GitManager(workspace=temp_workspace)
        result = manager.is_available
        assert isinstance(result, bool)

    def test_generate_commit_message_custom(self, temp_workspace):
        from modules.git_manager import GitManager
        manager = GitManager(workspace=temp_workspace)
...
class TestLogger:
    ...
    def test_structured_logger_init(self, temp_workspace):
    ...
    def test_structured_logger_debug(self, temp_workspace):
        from utils.logger import StructuredLogger
    ...
    def test_get_json_log_path(self, temp_workspace):
        from utils.logger import StructuredLogger
        logger = StructuredLogger(name="test", log_dir=temp_workspace)
    ...
    def test_get_recent_logs(self, temp_workspace):
        from utils.logger import StructuredLogger
        logger = StructuredLogger(name="test", log_dir=temp_workspace)
        logger.info("消息1")
        logger.info("消息2")
...
class TestCache:
    ...
    def test_cache_layer_init(self, temp_workspace):
    ...
    def test_cache_code(self, temp_workspace):
        from utils.cache import CacheLayer
        cache_dir = Path(temp_workspace) / ".cache"
        cache = CacheLayer(cache_dir=str(cache_dir))
    ...
    def test_cleanup_expired(self, temp_workspace):
        from utils.cache import CacheLayer
        cache_dir = Path(temp_workspace) / ".cache"
        cache = CacheLayer(cache_dir=str(cache_dir))
