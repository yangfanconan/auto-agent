...
class TestTaskPlan:
    ...
    def test_plan_progress(self):
        """测试进度计算"""
        subtasks = [
            SubTask(id="t1", name="T1", description="D1", task_type=TaskType.CODE_GENERATION, progress=50.0),
            SubTask(id="t2", name="T2", description="D2", task_type=TaskType.TESTING, progress=100.0),
        ]
    ...
    def test_completed_count(self):
        """测试完成计数"""
        subtasks = [
            SubTask(id="t1", name="T1", description="D1", task_type=TaskType.CODE_GENERATION, status="completed"),
            SubTask(id="t2", name="T2", description="D2", task_type=TaskType.TESTING, status="completed"),
            SubTask(id="t3", name="T3", description="D3", task_type=TaskType.TESTING, status="pending"),
        ]
    ...
    def test_failed_count(self):
        """测试失败计数"""
        subtasks = [
            SubTask(id="t1", name="T1", description="D1", task_type=TaskType.CODE_GENERATION, status="failed"),
            SubTask(id="t2", name="T2", description="D2", task_type=TaskType.TESTING, status="completed"),
            SubTask(id="t3", name="T3", description="D3", task_type=TaskType.TESTING, status="failed"),
        ]
...
class TestTaskParser:
    ...
    def test_match_task_template_cli(self):
        """测试匹配 CLI 工具模板"""
        parser = TaskParser()
        template = parser._match_task_template("开发一个命令行工具", [TaskType.PROJECT_INIT])
    ...
    def test_match_task_template_python_lib(self):
        """测试匹配 Python 库模板"""
        parser = TaskParser()
        template = parser._match_task_template("创建一个 Python 库", [TaskType.PROJECT_INIT])
    ...
    def test_match_task_template_refactor(self):
        """测试匹配重构模板"""
        parser = TaskParser()
        template = parser._match_task_template("审查并优化代码", [TaskType.CODE_REVIEW])
    ...
    def test_get_dependencies(self):
        ...
        parser = TaskParser()
        subtasks = [
            SubTask(id="task_001", name="T1", description="D1", task_type=TaskType.CODE_GENERATION),
            SubTask(id="task_002", name="T2", description="D2", task_type=TaskType.TESTING),
        ]
        
        deps = parser._get_dependencies(subtasks, ["task_001", "task_002", "task_003"])
    ...
    def test_generate_subtasks_with_project_type(self):
        """测试带项目类型生成子任务"""
        parser = TaskParser()
        subtasks = parser._generate_subtasks("创建项目", [TaskType.PROJECT_INIT], "web_api")
