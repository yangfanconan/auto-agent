...
class CacheLayer:
    ...
    def cleanup_expired(self) -> int:
        ...
        expired_keys = [
            k for k,
            v in self._memory_cache.items() if v.is_expired()]
    ...
    def cache_code(self, description: str, code: str,
    ...
    def get_cached_code(self, description: str,
    ...
    def cache_tool_response(self, tool_name: str,
    ...
    def get_cached_tool_response(
            self, tool_name: str, prompt: str) -> Optional[str]:
        """获取缓存的工具响应"""
        key = f"tool:{tool_name}:{self._get_key(prompt)}"
        data = self.get(key)
...
def get_cached_code(description: str,
                    language: str = "python") -> Optional[str]:
    """便捷函数：获取缓存代码"""
