...
class TaskTracker:
    ...
    def update_subtask_progress(
            self, plan_id: str, subtask_id: str, progress: float, log: Optional[str] = None):
        """更新子任务进度"""
        if plan_id not in self._plans:
            return
    ...
    def complete_subtask(self, plan_id: str, subtask_id: str,
    ...
    def _get_subtask(self, plan: TaskPlan,
                     subtask_id: str) -> Optional[SubTask]:
        """获取子任务"""
    ...
    def generate_briefing(self, plan_id: str) -> str:
        ...
        for subtask in plan.subtasks:
            status_icon = {
                ...
                "⏳")
            lines.append(
                f"- {status_icon} {subtask.name}: {subtask.status} ({subtask.progress:.0f}%)")
