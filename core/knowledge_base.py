...
class KnowledgeBase:
    ...
    def index_project(self, file_patterns: Optional[List[str]] = None) -> ProjectIndex:
        """
        索引项目结构和代码
            ...
            try:
                ...
                for node in ast.walk(tree):
                    if isinstance(node, ast.ClassDef):
                        class_info = self._extract_class_info(file_path, node, content)
                        file_info["classes"].append(class_info["name"])
                        self.index.classes[class_info["name"]] = class_info
                    
                    elif isinstance(node, ast.FunctionDef):
                        func_info = self._extract_function_info(file_path, node, content)
    ...
    def _extract_class_info(self, file_path: Path, node: ast.ClassDef, content: str) -> CodeInfo:
        """提取类信息"""
    ...
    def _extract_function_info(self, file_path: Path, node: ast.FunctionDef, content: str) -> CodeInfo:
        """提取函数信息"""
        docstring = ast.get_docstring(node)
    ...
    def search(self, query: str, search_type: str = "all") -> List[Dict]:
        ...
        if search_type in ["all", "class"]:
            for name, info in self.index.classes.items():
                if query_lower in name.lower() or (info.docstring and query_lower in info.docstring.lower()):
                    results.append({
                        "type": "class",
                        "name": name,
                        "file": info.file_path,
...
未找到与问题直接相关的内容，请尝试更具体的关键词。"""
    ...
    def save_index(self, path: Optional[str] = None):
        ...
        save_path = Path(path) if path else self.project_path / ".knowledge_index.json"
    ...
    def load_index(self, path: Optional[str] = None) -> bool:
        """从文件加载索引"""
        load_path = Path(path) if path else self.project_path / ".knowledge_index.json"
        ...
        try:
            ...
            self.index = ProjectIndex(
                ...
                classes={k: CodeInfo(**v) for k, v in data.get("classes", {}).items()},
                functions={k: CodeInfo(**v) for k, v in data.get("functions", {}).items()},
