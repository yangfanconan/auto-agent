    ...
    def execute_plan(self, plan: TaskPlan) -> bool:
        ...
        try:
            ...
            while len(completed) + len(failed) < len(plan.subtasks):
                ...
                if not ready_tasks:
                    ...
                    if failed:
                        ...
                        # 继续尝试执行其他不依赖失败任务的任务
                        remaining = [
                            t for t in plan.subtasks if t.id not in completed and t.id not in failed]
                        if not remaining:
                            break
                    else:
            ...
            if all_completed:
                ...
                self.tracker.complete_plan(plan.id)
                self.logger.warning(
                    f"任务计划部分完成：{len(completed)}/{len(plan.subtasks)}")
                return len(failed) == 0
...
class AutoAgent:
    ...
    def _register_default_handlers(self):
        ...
        self.scheduler.register_handler(
            TaskType.ENVIRONMENT_SETUP, env_handler)
